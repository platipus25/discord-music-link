...
class iTunes():
    ...
    def search(self, query):
        ...
        q = urllib.parse.parse_qs(url.query).get('i') if url.netloc == "music.apple.com" else None
...
def parse_youtube(body):
    ...
    results = [{
        ...
        "artist": ', '.join(artist['name'] for artist in result['artists']),
    } for result in body if result['resultType'] == "song"
    ]
...
def parse_apple_music(body):
    ...
    } for result in body if 'kind' in result and result['kind'] == 'song']
...
"""
                    ...
                    await message.channel.send(response, reference=message, mention_author=False, suppress_embeds=False)
